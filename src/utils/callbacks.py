...
def get_callbacks(config, X_train):
    ...
    with file_writer.as_default():
        images=np.reshape(X_train[20:60], (-1,28,28,1))  ### <<< 20,28,28,1
        tf.summary.image("20 image samples",images, max_outputs=25, step=0)

    params=config["params"]
    early_cb = tf.keras.callbacks.EarlyStopping(patience=params["patience"], restore_best_weights=params["restore_best_weights"])
    
    artifacts= config["artifacts"]
    CKPT_dir= os.path.join(artifacts["artifacts_dir"], artifacts["checkpoint_dir"])
    os.makedirs(CKPT_dir, exist_ok=True)
    
    CKPT_path=os.path.join(CKPT_dir, "ckpt_model.h5")

    checkpoint_cb =tf.keras.callbacks.ModelCheckpoint(CKPT_path, save_best_only=True )

    return [tensorboard_cb, early_cb, checkpoint_cb]